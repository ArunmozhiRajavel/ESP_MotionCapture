    ...
    case HTTP_CODE_OK:  ///< OK (Start Update)
        if(len > 0) {
            ...
            if(!startUpdate) {
                ...
                if(!spiffs) {
                    uint8_t buf[4];
					DEBUG_HTTP_UPDATE("[httpUpdate] Inside step 1\n");
					...
					DEBUG_HTTP_UPDATE("[httpUpdate] magicFlashChipSize - %d\n",bin_flash_size);
					DEBUG_HTTP_UPDATE("[httpUpdate] FlashChipRealSize - %d\n",ESP.getFlashChipRealSize());
...
bool ESP8266HTTPUpdate::runUpdate(Stream& in, uint32_t size, String md5, int command)
{
    ...
    StreamString error;
	DEBUG_HTTP_UPDATE("[httpUpdate] Entered runUpdate function\n");
    if(!Update.begin(size, command)) {
		...
		DEBUG_HTTP_UPDATE("[httpUpdate] Update.begin success\n");
	}
    if(md5.length()) {
    ...
    if(Update.writeStream(in) != size) {
		...
		DEBUG_HTTP_UPDATE("[httpUpdate] Update.writeStream success\n");
	}
    ...
    if(!Update.end()) {
		...
		DEBUG_HTTP_UPDATE("[httpUpdate] Update.end success\n");
	}
    return true;
}
